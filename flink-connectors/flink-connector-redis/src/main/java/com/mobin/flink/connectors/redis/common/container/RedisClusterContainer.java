package com.mobin.flink.connectors.redis.common.container;
...
public class RedisClusterContainer implements RedisCommandsContainer, Closeable {
    ...
    @Override
    public String hget(String additionalKey, String field) {
        try {
            return jedisCluster.hget(additionalKey, field);
        } catch (Exception e) {
            if (LOG.isErrorEnabled()) {
                LOG.error("Cannot send Redis message with command HGET to hash {} of key {} error message {}",
                        additionalKey, field, e.getMessage());
            }
            throw e;
        }
    }
